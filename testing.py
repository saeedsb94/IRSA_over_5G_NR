...
# Try to estimate the channel for the first slot using the known data
# h_hat = tf.math.divide_no_nan(y_combined[0, :, :], symbols_rg_store[0])
phi_hat=tf.math.angle(tf.math.reduce_sum( y_combined[0, :, :]*tf.math.conj(symbols_rg_store[0])))# I estimate only the phase no need to normalize
h_est = tf.complex(tf.cos(phi_hat), tf.sin(phi_hat))
